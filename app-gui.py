...
class StartPage(tk.Frame):
        ...
        def __init__(self, parent, controller):
            ...
            button3 = tk.Button(self, text="Edit Users", fg="#ffffff", bg="#263942",command=lambda: self.controller.show_frame("PageFive"))
...
class PageFive(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.dropdown.config(bg="lightgrey")
        self.dropdown["menu"].config(bg="lightgrey")
        self.buttonext = tk.Button(self, text="Delete", command=self.delete_user, fg="#ffffff", bg="#263942")
        self.dropdown.grid(row=0, column=1, ipadx=8, padx=10, pady=10)
        self.buttoncanc.grid(row=1, ipadx=5, ipady=4, column=0, pady=10)
        self.buttonext.grid(row=1, ipadx=5, ipady=4, column=1, pady=10)

    def delete_user(self):
        if self.menuvar.get() == "NO":
            messagebox.showerror("ERROR", "Name cannot be 'None'")
            return
        self.controller.active_name = self.menuvar.get()
        names.remove(self.menuvar.get())
        
        try:
            os.remove(f"data\classifiers\{self.menuvar.get()}_classifier.xml")
        except FileNotFoundError:
            try:
                shutil.rmtree(f"data\{self.menuvar.get()}")
            except FileNotFoundError:
                messagebox.showerror("EROOR", "No such file or directory!!!")
        # self.controller.show_frame("PageFour")
